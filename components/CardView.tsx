...
export const CardView: React.FC<{ links: CardLinks }> = ({ links }) => {
  ...
  return (
    <div className="p-5 bg-[var(--tint)] border border-[var(--line)] rounded-2xl space-y-4">
      ...
      <div className="rounded-xl overflow-hidden border border-[var(--line)] bg-white">
        <iframe
          title="บัตรแพ้ยา"
          src={links.embedUrl}
          className="w-full"
      ...
      <div className="flex flex-col sm:flex-row items-center gap-5">
        <div className="bg-white p-2 rounded-xl border border-[var(--line)] shrink-0">
          {qr ? (
            <img src={qr} alt="QR ตรวจความแท้บัตร" className="w-32 h-32" />
          ) : (
            <div className="w-32 h-32 flex items-center justify-center text-[var(--faint)]">
        ...
        <div className="flex-1 min-w-0 space-y-3 w-full">
          <p className="text-[11px] text-[var(--label)] leading-relaxed">
            สแกน QR เพื่อตรวจความแท้ของบัตรด้วย public key (ใครก็ตรวจได้ ไม่ต้องเชื่อ server)
          </p>
          <div className="flex items-center gap-2">
            <code className="flex-1 min-w-0 truncate text-[10px] text-[var(--label)] font-mono bg-[var(--surface)] border border-[var(--line)] rounded-lg px-2 py-1.5">
              {links.verifyUrl}
            </code>
            <button
              onClick={copy}
              className="shrink-0 p-1.5 rounded-lg border border-[var(--line)] bg-[var(--surface)] text-[var(--muted)] hover:text-[var(--primary)]"
          ...
          <div className="flex flex-wrap gap-2">
            <a
              ...
              className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold bg-[var(--surface)] hover:bg-[var(--bg)] border border-[var(--line)] rounded-xl text-[var(--ink)]"
            >
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold bg-[var(--primary)] hover:bg-[var(--primary-hover)] rounded-xl text-white"
            >
